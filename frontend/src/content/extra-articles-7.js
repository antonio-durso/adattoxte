// Articoli pilastro del piano editoriale (Fase 3, mappa SEO 90 giorni) — pubblicati il 26/08/2026
export const extraArticles7 = [
  {
    slug: 'test-ansia-gad-7',
    title: "Test ansia GAD-7: cos'è e come leggere il risultato",
    keyword: 'test ansia gad 7',
    metaDescription: "Il test GAD-7 misura i sintomi del disturbo d'ansia generalizzata in 7 domande. Scopri come funziona, come si interpreta e quando chiedere aiuto.",
    date: '2026-08-26',
    faq: [
      { q: "Il test GAD-7 è una diagnosi?", a: "No: il GAD-7 è uno strumento di screening che misura la frequenza dei sintomi ansiosi nelle ultime 2 settimane. Solo un professionista può formulare una diagnosi dopo un colloquio approfondito." },
      { q: 'Quanto tempo richiede il test?', a: 'Circa 2-3 minuti: sono 7 domande a risposta multipla su una scala da 0 a 3. Il risultato è immediato e orientativo.' },
      { q: 'Cosa fare se il punteggio è alto?', a: 'Se il punteggio indica un livello moderato o elevato di ansia, ti consigliamo di parlare con uno psicologo: un percorso mirato può ridurre sensibilmente i sintomi in poche settimane.' },
    ],
    body: `<p>Il test GAD-7 è uno dei questionari più usati al mondo per misurare l'ansia. In sole 7 domande valuta la frequenza dei sintomi ansiosi nelle ultime due settimane e fornisce un punteggio orientativo da 0 a 21. È uno strumento di screening validato dalla ricerca clinica, non una diagnosi: lo usiamo su Adatto x Te per aiutarti a capire come stai e a trovare il supporto giusto.</p>

<h2>Come funziona il GAD-7</h2>
<p>Il questionario chiede con quale frequenza, nelle ultime 2 settimane, ti hanno disturbato 7 problemi tipici dell'ansia: sentirsi nervosi, non riuscire a smettere di preoccuparsi, avere difficoltà a rilassarsi, essere irritabili, provare paura che possa accadere qualcosa di terribile. Per ogni domanda si sceglie tra "mai" (0), "alcuni giorni" (1), "più della metà dei giorni" (2) e "quasi ogni giorno" (3).</p>
<p>Il punteggio totale si colloca in quattro fasce: 0-4 ansia minima, 5-9 lieve, 10-14 moderata, 15-21 elevata. Ricorda: il punteggio è un punto di partenza, non un verdetto. Molte persone con punteggi moderati o elevati migliorano in modo significativo con un percorso di supporto psicologico.</p>

<h2>Dove fare il test</h2>
<p>Puoi fare il test GAD-7 gratuitamente sul nostro sito: ti basta accedere alla pagina dei <a href="/test">test di benessere</a> e rispondere alle domande in modo sincero. Alla fine ricevi subito il tuo livello e, se lo desideri, il suggerimento di un professionista specializzato in ansia e stress. Non è richiesta alcuna registrazione per provarlo.</p>

<h2>Quando chiedere aiuto</h2>
<p>Se il test evidenzia un livello moderato o elevato, o se l'ansia interferisce con il sonno, il lavoro, lo studio o le relazioni, è il momento di parlare con un professionista. Sul nostro <a href="/terapeuti">catalogo</a> trovi psicologi iscritti all'albo specializzati in ansia e stress, disponibili anche in videochiamata.</p>

<h2>Domande frequenti</h2>
<details><summary>Il test GAD-7 è una diagnosi?</summary><p>No: è uno strumento di screening. Solo un professionista può formulare una diagnosi dopo un colloquio approfondito.</p></details>
<details><summary>Quanto tempo richiede il test?</summary><p>Circa 2-3 minuti, con risultato immediato e orientativo.</p></details>
<details><summary>Cosa fare se il punteggio è alto?</summary><p>Parla con uno psicologo: un percorso mirato può ridurre sensibilmente i sintomi in poche settimane.</p></details>

<p class="muted small"><em>Disclaimer: questo articolo ha scopo informativo e non sostituisce una valutazione professionale. Fonte: Spitzer RL, Kroenke K, Williams JBW, Löwe B. A brief measure for assessing generalized anxiety disorder: the GAD-7. Arch Intern Med 2006 (<a href="https://pubmed.ncbi.nlm.nih.gov/16717171/" target="_blank" rel="noopener">PubMed</a>).</em></p>`,
  },
  {
    slug: 'test-phq-9-umore',
    title: 'Test PHQ-9: la scala per valutare il tuo umore',
    keyword: 'test depressione phq 9 umore',
    metaDescription: "Il PHQ-9 valuta la severità dei sintomi depressivi con 9 domande. Scopri come funziona, come si interpreta il punteggio e quando chiedere aiuto.",
    date: '2026-08-27',
    faq: [
      { q: 'Il PHQ-9 diagnostica la depressione?', a: 'No: è uno strumento di screening per valutare la frequenza dei sintomi depressivi nelle ultime 2 settimane. La diagnosi richiede una valutazione professionale completa.' },
      { q: 'Qual è un punteggio normale?', a: 'Un punteggio fino a 4 indica sintomi minimi; da 5 a 9 lievi; da 10 a 14 moderati; oltre 15 severi. Il significato va sempre contestualizzato dal professionista.' },
      { q: 'Il test include la domanda sull\'autolesionismo: come gestirla?', a: 'La domanda 9 riguarda pensieri di autolesionismo: se rispondi affermativamente, ti invitiamo a non restare solo e a contattare subito il 112/118 o il 1522.' },
    ],
    body: `<p>Il PHQ-9 è il questionario più diffuso al mondo per valutare i sintomi depressivi. Con 9 domande misura quanto spesso, nelle ultime due settimane, hai sperimentato umore basso, perdita di interesse, stanchezza, difficoltà di concentrazione e altri sintomi tipici. Il punteggio, da 0 a 27, è uno strumento di screening validato dalla ricerca: ti aiuta a capire come stai, ma non sostituisce la valutazione di un professionista.</p>

<h2>Come funziona il PHQ-9</h2>
<p>Ogni domanda valuta un sintomo: scarso interesse o piacere nelle attività, sentirsi giù o senza speranza, disturbi del sonno, stanchezza, appetito, senso di fallimento, difficoltà di concentrazione, lentezza o agitazione, e pensieri di autolesionismo. Le risposte vanno da "mai" (0) a "quasi ogni giorno" (3). Il totale colloca il quadro in una fascia: 0-4 minima, 5-9 lieve, 10-14 moderata, 15-27 severa.</p>

<h2>Dove fare il test</h2>
<p>Il test PHQ-9 è disponibile gratuitamente nella pagina dei <a href="/test">test di benessere</a> di Adatto x Te, insieme al GAD-7 per l'ansia. Risultato immediato e suggerimento del professionista più adatto, senza obbligo di registrazione.</p>

<h2>Quando chiedere aiuto</h2>
<p>Se il punteggio è moderato o elevato, o se ti riconosci in questi sintomi da più di due settimane, parlare con uno psicologo può fare una grande differenza. Sul <a href="/terapeuti">catalogo</a> trovi professionisti specializzati in ansia e depressione, disponibili anche online.</p>

<h2>Domande frequenti</h2>
<details><summary>Il PHQ-9 diagnostica la depressione?</summary><p>No: è uno screening. La diagnosi richiede una valutazione professionale completa.</p></details>
<details><summary>Qual è un punteggio normale?</summary><p>Fino a 4 sintomi minimi; oltre 15 severi. Il significato va contestualizzato dal professionista.</p></details>
<details><summary>Il test include la domanda sull'autolesionismo: come gestirla?</summary><p>Se rispondi affermativamente, non restare solo: contatta subito il 112/118 o il 1522.</p></details>

<p class="muted small"><em>Disclaimer: questo articolo ha scopo informativo e non sostituisce una valutazione professionale. Fonte: Kroenke K, Spitzer RL, Williams JBW. The PHQ-9: validity of a brief depression severity measure. J Gen Intern Med 2001 (<a href="https://pubmed.ncbi.nlm.nih.gov/11556941/" target="_blank" rel="noopener">PubMed</a>).</em></p>`,
  },
  {
    slug: 'gestire-ansia-concorsi-pubblici',
    title: 'Ansia da concorso: come gestirla con la psicologia',
    keyword: 'ansia da concorso preparazione mentale',
    metaDescription: "L'ansia da concorso pubblico è normale ma gestibile. Tecniche di preparazione mentale, gestione dello stress da studio e quando chiedere aiuto a uno psicologo.",
    date: '2026-08-28',
    faq: [
      { q: 'Uno psicologo può davvero aiutare con l\'ansia da concorso?', a: 'Sì: insegna tecniche di gestione dell\'ansia, concentrazione e simulazione mentale, e accompagna nei mesi di preparazione mantenendo costanza e lucidità il giorno della prova.' },
      { q: 'Le tecniche funzionano anche per i quiz e le prove a risposta multipla?', a: 'Sì: gestione del tempo, respirazione e autoistruzioni aiutano sia nelle prove scritte sia nei quiz, dove l\'ansia può indurre errori evitabili.' },
      { q: 'Quanto dura un percorso per l\'ansia da concorso?', a: 'Solitamente 4-8 incontri, concordati con lo psicologo in base al punto di partenza e alla data della prova.' },
    ],
    body: `<p>L'ansia da concorso pubblico è una delle più comuni e anche una delle più gestibili: mesi di studio, pressione sociale e paura di sbagliare possono attivare il sistema di allarme del cervello. La buona notizia è che la preparazione mentale si allena come quella tecnica, e la psicologia offre strumenti concreti per arrivare al giorno della prova lucidi e sicuri.</p>

<h2>Perché il concorso mette così tanta ansia</h2>
<p>Il concorso concentra in un'unica prova anni di investimento: la mente legge la situazione come una minaccia e risponde con attivazione fisiologica (tachicardia, tensione, pensieri negativi). Questo è normale: il problema nasce quando l'ansia supera la soglia utile e compromette memoria e concentrazione proprio quando servono.</p>

<h2>Tecniche di preparazione mentale</h2>
<p>Tra le strategie più efficaci: la respirazione diaframmatica lenta (4-6 respiri al minuto) per spegnere la risposta d'allarme; la simulazione realistica della prova (tempo cronometrato, ambiente silenzioso) per abituare il cervello alla situazione; le autoistruzioni positive ("ho studiato, so quello che serve"); e la gestione delle pause per recuperare attenzione. Approfondisci nel nostro approfondimento sull'ansia da concorso nella landing dedicata: <a href="/psicologo-concorsi-pubblici">psicologo per concorsi pubblici</a>.</p>

<h2>Quando chiedere aiuto</h2>
<p>Se l'ansia ti blocca nello studio, ti tiene sveglio la notte o hai già fallito prove per tensione, un percorso con uno psicologo specializzato in preparazione ai concorsi può cambiare l'esito. Sul <a href="/terapeuti">catalogo</a> trovi professionisti dedicati, anche in videochiamata.</p>

<h2>Domande frequenti</h2>
<details><summary>Uno psicologo può davvero aiutare con l'ansia da concorso?</summary><p>Sì: tecniche di gestione dell'ansia, concentrazione e simulazione mentale, con accompagnamento nei mesi di preparazione.</p></details>
<details><summary>Le tecniche funzionano anche per i quiz?</summary><p>Sì: gestione del tempo, respirazione e autoistruzioni aiutano anche nelle prove a risposta multipla.</p></details>
<details><summary>Quanto dura un percorso?</summary><p>Solitamente 4-8 incontri, in base al punto di partenza e alla data della prova.</p></details>

<p class="muted small"><em>Disclaimer: questo articolo ha scopo informativo e non sostituisce una valutazione professionale.</em></p>`,
  },
  {
    slug: 'ansia-da-prestazione-sportiva',
    title: 'Ansia da prestazione sportiva: quando la mente frena',
    keyword: 'ansia da prestazione sportiva psicologia',
    metaDescription: "L'ansia da prestazione può bloccare anche gli atleti più preparati. Scopri le tecniche di psicologia dello sport per gestirla in gara e in allenamento.",
    date: '2026-08-29',
    faq: [
      { q: 'L\'ansia da prestazione riguarda solo i professionisti?', a: 'No: anche gli amatori e chi fa sport per benessere possono bloccarsi in gara o in momenti importanti. Il supporto psicologico è utile a ogni livello.' },
      { q: 'Come si gestisce l\'ansia pochi minuti prima della gara?', a: 'Respirazione lenta (4-6 atti al minuto), routine pre-gara costante, focus su ciò che dipende da te e visualizzazione del gesto tecnico.' },
      { q: 'Quanto dura un percorso di psicologia dello sport?', a: 'Spesso bastano 4-8 incontri mirati per acquisire tecniche di gestione dell\'attivazione e della concentrazione.' },
    ],
    body: `<p>L'ansia da prestazione è la paura di non riuscire nel momento in cui conta: può far tremare le mani, accelerare il cuore e "spegnere" le capacità che in allenamento funzionano perfettamente. È uno dei motivi principali per cui gli atleti si rivolgono alla psicologia dello sport: non per debolezza, ma perché la mente è parte della prestazione quanto il corpo.</p>

<h2>Come riconoscerla</h2>
<p>I segnali tipici: tensione muscolare eccessiva, respiro corto, pensieri negativi ripetitivi ("se sbaglio..."), paura del giudizio, evitamento delle gare o calo della prestazione solo in competizione. Quando l'attivazione supera la soglia ottimale, la prestazione cala proprio per l'eccesso di controllo o di allarme.</p>

<h2>Le tecniche della psicologia dello sport</h2>
<p>Tra le strategie validate: il controllo dell'attivazione con respirazione e rilassamento progressivo; la visualizzazione del gesto tecnico e del piano gara; le routine pre-gara che ancorano l'attenzione; il focus sul processo invece che sul risultato; e la ristrutturazione dei pensieri ("è adrenalina, non pericolo"). Approfondisci nella landing <a href="/psicologo-sport">psicologia dello sport</a>.</p>

<h2>Quando chiedere aiuto</h2>
<p>Se la paura della gara ti limita da tempo, un percorso breve con uno psicologo dello sport può riportarti a dare il meglio. Sul <a href="/terapeuti">catalogo</a> trovi professionisti specializzati, anche online.</p>

<h2>Domande frequenti</h2>
<details><summary>L'ansia da prestazione riguarda solo i professionisti?</summary><p>No: blocca anche gli amatori. Il supporto psicologico è utile a ogni livello.</p></details>
<details><summary>Come si gestisce l'ansia prima della gara?</summary><p>Respirazione lenta, routine pre-gara, focus su ciò che dipende da te e visualizzazione.</p></details>
<details><summary>Quanto dura un percorso?</summary><p>Spesso bastano 4-8 incontri mirati.</p></details>

<p class="muted small"><em>Disclaimer: questo articolo ha scopo informativo e non sostituisce una valutazione professionale.</em></p>`,
  },
  {
        slug: 'tradimento-ricostruire-fiducia',
        title: 'Tradimento: come ricostruire la fiducia nella coppia',
        keyword: 'tradimento ricostruire fiducia',
        metaDescription: 'Dopo un tradimento si può ricostruire la fiducia? Le fasi della crisi, gli errori da evitare, il ruolo della terapia di coppia e come decidere se ripartire.',
        date: '2026-08-31',
        faq: [
          { q: 'Si può ricostruire la fiducia dopo un tradimento?', a: 'Sì, ma richiede tempo, impegno e la volontà di entrambi di lavorare sulla relazione: la fiducia non torna da sola, si costruisce con gesti coerenti nel tempo.' },
          { q: 'Quanto dura la crisi dopo un tradimento?', a: 'Le fasi emotive (shock, rabbia, tristezza) possono durare mesi; il percorso di ricostruzione, con o senza terapia, richiede in genere dai sei mesi a un anno e più.' },
          { q: 'La terapia di coppia serve davvero in questi casi?', a: 'Sì: uno psicologo aiuta a trasformare il dolore in comunicazione, a capire le cause profonde e a decidere in modo consapevole se e come ripartire.' },
        ],
        body: `<p>Scoprire un tradimento è uno degli eventi più dolorosi in una relazione: alla ferita si mescolano rabbia, senso di colpa, vergogna e la domanda che torna ossessiva — "si può ancora avere fiducia in noi?". Non esiste una risposta unica, ma esiste un percorso: capire cosa è successo, cosa significa per la coppia e se entrambi vogliono davvero ricostruire.</p>
    <h2>La tempesta emotiva: cosa succede subito dopo</h2>
    <p>Le prime settimane sono dominate da emozioni intense e alternate: shock, incredulità, rabbia, tristezza, ma anche momenti di apparente normalità. Ogni partner vive il proprio dolore — chi ha tradito prova spesso colpa e vergogna, chi è stato tradito ferita e senso di inadeguatezza. Prendersi il tempo per elaborare, senza decisioni affrettate né sotto pressione, è il primo passo utile.</p>
    <h2>Gli errori da evitare</h2>
    <ul><li><strong>Decidere nella tempesta</strong>: né "chiudiamo tutto" detto nella rabbia, né "ripartiamo come se nulla fosse" detto per paura di restare soli.</li>
    <li><strong>Il tribunale dei dettagli</strong>: chiedere e raccontare ogni singolo particolare non ricostruisce la fiducia, alimenta la sofferenza.</li>
    <li><strong>Coinvolgere tutti</strong>: famiglia e amici, per quanto in buona fede, rendono più difficile la comunicazione diretta tra i partner.</li>
    <li><strong>Saltare le cause</strong>: se la relazione è arrivata a quel punto, di solito c'è un problema di fondo — distanza emotiva, bisogni inespressi, difficoltà di comunicazione — che senza affrontarlo si ripresenterà.</li>
    </ul>
    <h2>Si può ricostruire la fiducia?</h2>
    <p>Sì, ma la fiducia non torna da sola: si ricostruisce con gesti coerenti e ripetuti nel tempo. Chi ha tradito deve assumersi la responsabilità senza giustificazioni, rispondere alle domande necessarie e rendere la relazione più trasparente; chi è stato tradito deve poter esprimere il dolore senza essere giudicato. È un lavoro lungo — spesso mesi — e non sempre la conclusione è restare insieme: a volte il vero risultato del percorso è separarsi con meno dolore e più lucidità.</p>
    <h2>Il ruolo della terapia di coppia</h2>
    <p>Uno psicologo specializzato offre uno spazio neutrale dove le emozioni possono essere dette senza accuse, aiuta a far emergere le cause profonde della crisi e accompagna la coppia nel decidere — ripartire o chiudere — con consapevolezza, non per impulso. Le sedute online si adattano anche a partner che vivono in città diverse.</p>
    <p>Se state attraversando questa fase, su <a href="/terapeuti">Adatto x Te</a> puoi trovare psicologi specializzati in terapia di coppia. <a href="/registrazione">Registrati gratuitamente</a> e inizia con una prima seduta conoscitiva.</p>
    <p class="muted small"><em>Disclaimer: questo articolo ha scopo informativo e non sostituisce una valutazione professionale.</em></p>`
  }
];
